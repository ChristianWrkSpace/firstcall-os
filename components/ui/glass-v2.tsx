...
export function GlassV2({
  children, className, accent = "neutral", depth = "surface", as: Component = "div",
  ...props
}: {
  children: ReactNode; className?: string; accent?: Accent; depth?: Depth; as?: ElementType;
}) {
  return (
    <Component className={cn("rounded-2xl ring-1 animate-spatial-rise", DEPTH_CLASS[depth], ACCENT_RING[accent], ACCENT_GLOW[accent], className)} {...props}>
      {children}
    </Component>
  );
}

export function PanelHeader({ title, sub, right, emoji, className }: { title: string; sub?: string; right?: ReactNode; emoji?: string; className?: string }) {
  return (
...
const CHIP_TONE: Record<ChipTone, string> = {
  neutral: "bg-white/[0.04] text-white/70", teal: "bg-[#5FBDB0]/10 text-[#5FBDB0]",
  amber: "bg-[#F59E0B]/10 text-[#F59E0B]", blue: "bg-[#6B8AD9]/10 text-[#6B8AD9]",
};

export function CountChip({ count, label, tone = "neutral" }: { count: number; label?: string; tone?: ChipTone }) {
  return (
    <span className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-medium tracking-[0.18em] uppercase backdrop-blur-md", CHIP_TONE[tone])}>
      <span className="tabular-nums">{count}</span>{label && <span>{label}</span>}
    </span>
  );
}
...
export function PageBackdrop({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div className={cn("relative min-h-screen overflow-hidden", className)}>
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute inset-0 animate-drift opacity-40" style={{ backgroundImage: "radial-gradient(circle at 18% 12%, rgba(107,138,217,0.10) 0%, transparent 38%), radial-gradient(circle at 82% 88%, rgba(95,189,176,0.08) 0%, transparent 42%)" }} />
        <svg className="absolute inset-0 w-full h-full opacity-[0.025]" aria-hidden="true">
          <filter id="noise-svg"><feTurbulence type="fractalNoise" baseFrequency="0.65" numOctaves="3" stitchTiles="stitch" /></filter>
