...
export const ROADMAP: RoadmapItem[] = [
  ...
  },
  {
    ...
    shipped_at: "2026-04-30",
    description:
      "Sign-out-everywhere-except-here for the current user + Owner-only force-sign-out for any org user. Lists all auth users with last sign-in time so stale accounts surface. All actions audit-logged (session.signout_others / session.force_signout).",
    features: [
      "Per-user 'sign out other devices' button",
      "Owner-only org user list with last sign-in age",
      "Force sign-out target user from EVERY device",
      "Self-protected: can't force sign-out yourself by accident",
      "All actions audit-logged for forensics",
    ],
  },
  {
    id: "security-activity-dashboard",
    ...
    shipped_at: "2026-04-30",
    description:
      "/settings/security-activity — auth events, MFA changes, rate-limit hits, secret rotations, backup drills surfaced from audit_logs. Risk-signal callouts (brute-force detection, MFA unenroll spikes, rate-limit abuse) plus per-action counts and rate-limit-key breakdown.",
    features: [
      "11 security action types color-coded with emojis",
      "Risk signals auto-flag brute-force / abuse / MFA removal patterns",
      "Per-rate-limit-key breakdown (which user/endpoint is hitting limits)",
      "rate_limit.hit now persisted to audit_logs (was in-memory only before)",
      "Window picker: 24h / 7d / 30d / 90d",
    ],
  },
  {
    id: "subs",
    title: "Subcontractor Portal Foundation",
    track: "core",
    status: "done",
    effort: "L",
    shipped_at: "2026-04-30",
    description:
      "Migration 026 + /subs page. Manage subs (recon, plumbers, electricians, abatement) with W-9 / EIN tracking + 1099-NEC YTD totals + per-job invoice logging. IRS $600 threshold automatically flags non-corp subs that need 1099-NEC.",
    features: [
      "subcontractors table: name, trade, contact, EIN/SSN last 4, is_corporation, W-9 path",
      "sub_invoices table: per-job, with payment method + receipt path",
      "YTD totals per sub computed live, 1099 threshold auto-flagged ($600)",
      "Trade datalist autocomplete (Reconstruction, Plumber, etc.)",
      "Linked from main nav as 🤝 Subs (STAFF role)",
  ...
  },
  {
    ...
    shipped_at: "2026-04-30",
    description:
      "/turing — meta-agent that reads recent agent_outcomes + system health and produces 5–8 concrete prioritized insights categorized as prompt_quality / process_friction / cost_efficiency / data_gap / wins. Read-only by design — Turing recommends, you act. Each run audit-logged.",
    features: [
      "lib/turing.ts: gathers per-agent outcome stats + recent deltas + health snapshot",
      "Runs through Opus (SMART tier) with strict tool-use schema for structured findings",
      "5 insight categories with severity (high/med/low) and color-coded UI",
      "Window picker: 7/14/30/90d",
      "Honest about sparse data — surfaces 'log more X' as a low-severity insight when needed",
    ],
  },
  {
