  ...
  return (
    <div className="min-h-screen bg-card">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        {scope.summary && (
          <section className="bg-card border border-edge2 rounded-xl p-6 mb-5">
            <h2 className="text-ink font-semibold mb-2">📋 Scope of Work</h2>
            <p className="text-ink-2 text-sm leading-relaxed mb-4">{scope.summary}</p>
        ...
        {estimates && estimates.length > 0 && (
          <section className="bg-card border border-edge2 rounded-xl p-6 mb-5">
            <h2 className="text-ink font-semibold mb-3">💰 Approved Estimate</h2>
                ...
                <div key={est.id}>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="text-ink-3 text-xs uppercase tracking-wide border-b border-edge2">
                      ...
                      <tbody>
                        {lines.map((l: any, i: number) => (
                          <tr key={i} className="border-b border-edge2/40">
                            <td className="py-1.5 text-ink-2 text-xs font-mono">
                              {l.xactimate_code ?? "—"}
                            </td>
                            <td className="py-1.5 text-ink">{l.description}</td>
                            <td className="py-1.5 text-right text-ink-2 font-mono text-xs">
                              {Number(l.quantity).toFixed(2)}
                            </td>
                            <td className="py-1.5 pl-2 text-ink-3 text-xs uppercase">
                              {l.unit}
                            </td>
                            <td className="py-1.5 text-right text-ink-2 font-mono text-xs">
                              {fmt(Number(l.unit_price))}
                            </td>
                            <td className="py-1.5 text-right text-ink font-mono text-xs">
                              {fmt(Number(l.line_total))}
                            </td>
                          </tr>
                        ))}
                        <tr>
                          <td colSpan={5} className="py-2 text-right text-ink-2 font-semibold">
                            Total
                          </td>
                          <td className="py-2 text-right text-ink font-mono text-base font-bold">
                            {fmt(total)}
                          </td>
                        </tr>
                      </tbody>
        ...
        {photoUrls.length > 0 && (
          <section className="bg-card border border-edge2 rounded-xl p-6 mb-5">
            <h2 className="text-ink font-semibold mb-3">📸 Site Photos</h2>
        ...
        {(legalDocs?.length ?? 0) > 0 && (
          <section className="bg-card border border-edge2 rounded-xl p-6 mb-5">
            <h2 className="text-ink font-semibold mb-3">⚖️ Legal Documents</h2>
            <ul className="flex flex-col gap-2">
              {(legalDocs as any[]).map((d) => {
                ...
                return (
                  <li
                    key={d.id}
                    className="flex items-center justify-between bg-shade rounded-lg px-3 py-2 text-sm gap-3"
                  >
        ...
        {docsWithUrls.length > 0 && (
          <section className="bg-card border border-edge2 rounded-xl p-6 mb-5">
            <h2 className="text-ink font-semibold mb-3">📑 Documents</h2>
            <ul className="flex flex-col gap-2">
                ...
                return (
                  ...
                  >
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener"
                      className="px-3 py-1.5 bg-cta hover:bg-cta-deep text-white text-xs rounded font-medium"
                    >
                      View
                    </a>
                  </li>
        ...
        {moistureDates.length > 0 && (
          <section className="bg-card border border-edge2 rounded-xl p-6 mb-5">
            <h2 className="text-ink font-semibold mb-3">💧 Moisture Readings</h2>
                      ...
                      <tbody>
                        {moistureByDate[date].map((r: any) => (
                          <tr key={r.id} className="border-b border-edge2/40">
                            <td className="py-1.5 text-ink">{r.room}</td>
...
function Field({
  ...
  return (
    <div>
      <p className="text-ink-3 text-xs uppercase tracking-wide mb-0.5">{label}</p>
      <p
        className={`text-ink ${capitalize ? "capitalize" : ""} ${mono ? "font-mono text-sm" : ""}`}
      >
        {value || "—"}
      </p>
    </div>
...
function Pill({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium bg-shade text-ink border border-edge2">
      {children}
    </span>
  );
}
