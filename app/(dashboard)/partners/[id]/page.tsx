    ...
    <div className="p-4 md:p-8 max-w-5xl">
      <div className="mb-6">
        ...
        <div className="flex items-baseline gap-3 mt-2 flex-wrap">
          ...
          {!partner.active && (
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-shade text-ink-2 uppercase">
              Inactive
            </span>
          )}
        </div>
        {partner.company && (
          <p className="text-ink-2 text-sm mt-1">{partner.company}</p>
        )}
        <div className="text-ink-3 text-xs mt-1.5 flex gap-3 flex-wrap">
          {partner.phone && (
            <a href={`tel:${partner.phone}`} className="hover:text-ink">
              {partner.phone}
            </a>
          )}
          {partner.email && (
            <a href={`mailto:${partner.email}`} className="hover:text-ink">
              {partner.email}
            </a>
          )}
        </div>
      ...
      <section className="glass-card p-6 mb-5">
        <h2 className="text-ink font-semibold mb-3">Referred Jobs</h2>
      ...
      {canLog && (
        <section className="glass-card p-6 mb-5">
          <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
            <div>
              <h2 className="text-ink font-semibold">Cash Payouts</h2>
          ...
          {(payouts?.length ?? 0) === 0 ? (
            <p className="text-ink-3 text-sm italic">No cash payouts logged.</p>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-edge2 text-ink-3 text-xs uppercase tracking-wide">
              ...
              <tbody>
                {(payouts as any[]).map((p) => (
                  ...
                  >
                    <td className="py-2.5 text-ink-2 text-xs">{p.occurred_on}</td>
                    <td className="py-2.5 text-ink-2 text-xs capitalize">{p.method}</td>
                    <td className="py-2.5 text-ink-2 text-xs font-mono">{p.reference ?? "—"}</td>
                    <td className="py-2.5 text-ink-3 text-xs">{p.notes ?? "—"}</td>
                    <td className="py-2.5 text-right font-mono text-xs text-ink">
      ...
      {canLog && (
        <section className="glass-card p-6">
          <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
            <div>
              <h2 className="text-ink font-semibold">Soft Investments</h2>
              <p className="text-ink-3 text-xs mt-0.5">
          ...
          {(investments?.length ?? 0) === 0 ? (
            <p className="text-ink-3 text-sm italic">No investments logged.</p>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-edge2 text-ink-3 text-xs uppercase tracking-wide">
              ...
              <tbody>
                {(investments as any[]).map((i) => (
                  <tr
                    ...
                    className="border-b border-edge2/60 last:border-0"
                  >
                    <td className="py-2.5 text-ink-2 text-xs">{i.occurred_on}</td>
                    <td className="py-2.5 text-ink-2 text-xs">{CATEGORY_LABEL[i.category] ?? i.category}</td>
                    <td className="py-2.5 text-ink-3 text-xs">{i.notes ?? "—"}</td>
                    <td className="py-2.5 text-right font-mono text-xs text-ink">
