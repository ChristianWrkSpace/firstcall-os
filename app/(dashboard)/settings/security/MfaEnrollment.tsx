...
const INPUT =
  "px-3 py-2 rounded-lg bg-shade border border-edge2 text-ink placeholder:text-ink-3 focus:outline-none focus:ring-2 focus:ring-cta text-base font-mono tracking-widest";
  ...
  if (!enrollment) {
    return (
      <div>
        ...
        {enrollmentError && (
          <p className="mt-2 text-red-700 text-xs">{enrollmentError}</p>
        )}
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-start">
        ...
        <div className="flex flex-col gap-3">
          <div>
            <p className="text-ink-2 text-sm font-semibold mb-1">1. Scan with your authenticator</p>
            <p className="text-ink-3 text-xs">
              Open Google Authenticator / 1Password / Authy and scan the QR.
              Or paste the secret manually:
            </p>
            <code className="block mt-2 bg-shade border border-edge2 rounded px-3 py-2 text-ink text-xs font-mono break-all">
              {enrollment.secret}
            </code>
          </div>
          ...
          <form action={verifyAction} className="flex flex-col gap-2">
            ...
            <p className="text-ink-2 text-sm font-semibold">2. Enter the 6-digit code</p>
            <div className="flex gap-2 items-center">
              ...
              <button
                ...
                className="px-3 py-2 text-ink-2 hover:text-ink text-sm"
              >
                Cancel
              </button>
            </div>
            {verifyState?.error && (
              <p className="text-red-700 text-xs">{verifyState.error}</p>
            )}
            {verifyState?.ok && (
              <p className="text-pine text-xs">✓ Verified! Refreshing…</p>
            )}
          </form>
        </div>
      </div>
