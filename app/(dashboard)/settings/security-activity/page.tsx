...
const ACTION_META: Record<string, { color: string; emoji: string }> = {
  "session.signout_others":  { color: "text-info",   emoji: "🚪" },
  "session.force_signout":   { color: "text-red-700",    emoji: "🚫" },
  "mfa.enrolled":            { color: "text-pine",  emoji: "🔐" },
  "mfa.unenrolled":          { color: "text-honey", emoji: "🔓" },
  "rate_limit.hit":          { color: "text-honey", emoji: "🚦" },
  "auth.failed_login":       { color: "text-red-700",    emoji: "❌" },
  "auth.password_reset":     { color: "text-info",   emoji: "🔑" },
  "customer.pii_redacted":   { color: "text-violet-700", emoji: "🗑️" },
  "secret.rotated":          { color: "text-pine",  emoji: "🔄" },
  "backup.drill_verify":     { color: "text-pine",  emoji: "🔍" },
  "backup.downloaded":       { color: "text-info",   emoji: "📥" },
  "default":                 { color: "text-ink-2",   emoji: "📜" },
};
    ...
    <div className="p-4 md:p-8 max-w-5xl">
      <div className="mb-6 flex items-end justify-between gap-3 flex-wrap">
        <div>
          ...
          <h1 className="text-2xl font-bold text-ink mt-2">Security Activity</h1>
          ...
          <ul className="space-y-1.5">
            {signals.map((s, i) => (
              <li key={i} className="flex items-start gap-2 text-sm">
                <span className={s.severity === "high" ? "text-red-700" : "text-honey"}>●</span>
        ...
        {Array.from(counts.entries())
          ...
          .map(([action, count]) => {
            ...
            return (
              <div
                key={action}
                className="bg-card border border-edge2 rounded-lg p-3 text-center"
              >
                <p className="text-xl">{meta.emoji}</p>
                <p className={`text-xl font-bold font-mono ${meta.color}`}>{count}</p>
                <p className="text-ink-3 text-[10px] mt-0.5 truncate">
                  {action.replace(/[._]/g, " ")}
                </p>
              </div>
            );
      ...
      {rateLimitKeys.size > 0 && (
        <div className="glass-card p-5 mb-6">
          <h2 className="text-ink font-semibold mb-3">🚦 Rate Limit Hits by Key</h2>
          <ul className="flex flex-col gap-1.5">
            {Array.from(rateLimitKeys.entries())
              ...
              .map(([key, count]) => (
                <li
                  key={key}
                  className="flex items-center justify-between text-xs px-3 py-1.5 bg-tint rounded"
                >
                  <code className="text-ink-2 truncate">{key}</code>
                  <span className="text-honey font-mono font-semibold">×{count}</span>
                </li>
              ))}
          </ul>
        </div>
      ...
      <div className="glass-card p-5">
        <h2 className="text-ink font-semibold mb-3">Event Stream</h2>
        {!events?.length ? (
          <p className="text-ink-3 text-sm italic">No security events in this window.</p>
        ) : (
          <ul className="flex flex-col gap-1.5 max-h-[500px] overflow-y-auto pr-1">
            {events.map((e: any) => {
              ...
              return (
                <li
                  key={e.id}
                  className="flex items-start gap-2 px-3 py-2 bg-tint border border-edge2/40 rounded-lg"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline justify-between gap-2 flex-wrap">
                      ...
                      <p className="text-ink-3 text-[10px] font-mono">{fmtAgo(e.created_at)}</p>
                    </div>
                    ...
                    {e.details && Object.keys(e.details).length > 0 && (
                      <pre className="text-ink-3 text-[10px] mt-1 overflow-x-auto">
