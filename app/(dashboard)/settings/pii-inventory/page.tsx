...
const CLASS_BADGE: Record<PiiField["classification"], string> = {
  ...
  contact:   "bg-orange-500/15 text-orange-300",
  claim:     "bg-purple-500/15 text-purple-300",
  ...
  if (me.role !== "owner" && me.role !== "manager") {
    return (
      <div className="p-8 max-w-2xl">
        <h1 className="text-2xl font-bold text-white">PII Inventory</h1>
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-white mt-2">🗂 PII Inventory</h1>
      ...
      <Section title="Retention Policy">
        <ul className="flex flex-col gap-2 text-sm text-zinc-300 list-disc list-inside">
          <li>
            <strong className="text-white">Financial records (invoices/payments):</strong>{" "}
            7 years per IRS Section 6001 — even if a customer requests deletion.
            We redact identifying columns but keep amount + dates.
          </li>
          <li>
            <strong className="text-white">Call recordings:</strong> 1 year max.
            Voice is biometric data under TX § 521.002. Auto-purge cron is a
            future task — for now, manual cleanup quarterly.
          </li>
          <li>
            <strong className="text-white">Legal documents (AOBs, demand letters):</strong>{" "}
            10 years — longer than other PII because litigation can surface late.
          </li>
          <li>
            <strong className="text-white">Photos & uploaded docs:</strong> 7
            years aligned with the related job's records, redacted on request
            (storage object delete + DB row redact).
          </li>
          <li>
            <strong className="text-white">Outreach leads:</strong> 3 years or
      ...
      <Section title="Cross-Border + Vendor Data Movement">
        <ul className="flex flex-col gap-2 text-sm text-zinc-300 list-disc list-inside">
          <li>
            <strong className="text-white">Anthropic (Claude):</strong> sends
          ...
          <li>
            <strong className="text-white">Deepgram:</strong> receives call
            audio for STT. Audio retained per Deepgram policy; we don't link
            their storage to ours.
          </li>
          <li>
            <strong className="text-white">ElevenLabs:</strong> receives
            assistant text for TTS — outbound only, no PII sent.
          </li>
          <li>
            <strong className="text-white">Stripe:</strong> receives customer
            email + payment amount + invoice number; no PII beyond what's
            needed for payment.
          </li>
          <li>
            <strong className="text-white">Resend:</strong> receives recipient
            email + branded message body for transactional sends.
          </li>
          <li>
            <strong className="text-white">Supabase + Vercel:</strong> primary
